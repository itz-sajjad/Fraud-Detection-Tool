...
Operating on a service-based business model, the company monetizes its services based on performance. Users pay a fixed fee contingent upon the successful detection of fraud in their transactions.

To accelerate customer acquisition in Brazil, the company has implemented an aggressive strategy. Under this approach:

The company earns 25% of the value for each successfully detected fraudulent transaction.
It earns 5% of the transaction value for each legitimate transaction mistakenly identified as fraud.
The company provides a full refund for any transaction identified as legitimate but later confirmed as fraudulent.
This strategy shifts the risk of fraud detection failure onto the company while incentivizing accurate fraud detection. Despite the relatively high fee charged upon successful detection (25%), clients benefit from reduced costs associated with accurately detected fraudulent transactions. Additionally, any losses resulting from errors in the anti-fraud service are covered by the Blocker Fraud Company.

For the company, success hinges on the precision and accuracy of the models developed by its Data Scientists. The more accurate the "Blocker Fraud" model, the higher the company's revenue potential. However, a low-accuracy model could result in significant losses.
